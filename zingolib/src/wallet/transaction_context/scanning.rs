...
impl TransactionContext {
            ...
            if let Some(t_bundle) = transaction.transparent_bundle() {
                for vout in &t_bundle.vout {
                    if let Some(taddr) = vout
                        .recipient_address()
                        .map(|raw_taddr| address_from_pubkeyhash(&self.config.chain, raw_taddr))
                    {
                            ...
                            if !transaction.outgoing_tx_data.is_empty() {
                                let outgoing_potential_receivers = [
                                    ...
                                    ua.transparent().map(|taddr| {
                                        address_from_pubkeyhash(&self.config.chain, *taddr)
                                    }),
        ...
        if let Some(t_bundle) = transaction.transparent_bundle() {
            for (n, vout) in t_bundle.vout.iter().enumerate() {
                if let Some(taddr) = vout.recipient_address() {
                    let output_taddr = address_from_pubkeyhash(&self.config.chain, taddr);
        ...
        D::Memo: zingo_traits::ToBytes<512>,
    {
        ...
        recover_and_record_domain_specific_outputs(
            &domain_tagged_outputs,
            outgoing_metadatas,
            &self.config.chain,
            status,
...
async fn recover_and_record_domain_specific_outputs<D: DomainWalletExt, P: Parameters>(
    ...
    params: &P,
    status: ConfirmationStatus,
    external_outgoing_viewing_key: D::OutgoingViewingKey,
    is_outgoing_transaction: &mut bool,
    mut addresses: impl Iterator<Item = &UnifiedAddress>,
) where
    <D as zcash_note_encryption::Domain>::Note: PartialEq + Clone,
    <D as zcash_note_encryption::Domain>::Recipient: zingo_traits::Recipient,
    <D as zcash_note_encryption::Domain>::Memo: ToBytes<512>,
{
        ...
        outgoing_metadatas.extend(
            ...
            >(
                ...
                Some((note, payment_address, memo_bytes)) => {
                    ...
                    *is_outgoing_transaction = true;
                    let address = payment_address.b32encode_for_network(params);
                    ...
                    match Memo::from_bytes(&memo_bytes.to_bytes()) {
                        Err(_) => None,
                        Ok(memo) => {
                            if addresses.any(|unified_address| {
                                [
