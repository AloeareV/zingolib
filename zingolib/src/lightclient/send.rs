...
impl LightClient {
        ...
        if let Some(proposal) = self.latest_proposal.read().await.as_ref() {
            match proposal {
                crate::lightclient::ZingoProposal::Transfer(transfer_proposal) => {
                    self.send_proposal_inner(transfer_proposal, DoSendProposedError::Calculation)
                        .await
                }
    ...
    async fn send_proposal_inner<FeeRule, NoteRef>(
        &self,
        transfer_proposal: &Proposal<FeeRule, NoteRef>,
        err_f: impl Fn(FeeRule::Error) -> DoSendProposedError,
