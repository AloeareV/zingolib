...
fn test_do_rescan() {
    ...
    let (regtest_manager, sender, recipient, child_process_handler, _client_builder) =
        scenarios::two_clients_one_saplingcoinbase_backed();
    tokio::runtime::Runtime::new().unwrap().block_on(async {
        // Give us some startup blocks, so we have access to some mining rewards.
        utils::increase_height_and_sync_client(&regtest_manager, &sender, 5).await;

        let recipient_sapling_address = get_base_address!(recipient, "sapling");
        let recipient_full_ua = get_base_address!(recipient, "unified");

        // send funds to orchard addresss
        let send_o_value = 600_000;
        let send_s_value = 400_000;
        let tos = vec![
            (&recipient_full_ua[..], send_o_value, None),
            (&recipient_sapling_address[..], send_s_value, None),
        ];
        sender.do_send(tos).await.unwrap();
    ...
    // This isn't needed as it's dropped at the end of the test anyway, but this makes
    // it clearer that we return this from the scenario maker because need to hold onto itr
    drop(child_process_handler);
}
